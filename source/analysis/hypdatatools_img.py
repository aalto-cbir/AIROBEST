...
def plot_hyperspectral( hypfilename, hypdata=None, hypdata_map=None, outputcommand=None, plotmode='default', plotbands=None ):
    """
    Create a figure with hypersectral image and return handle
...
def simulate_multispectral( hyp_infile, spectralsensitivityfile, out_pixelsize, out_multifilename=None, hypdata=None, hypdata_map=None, pixelsizereserve=None, max_DIVfraction=0.1, out_interleave='bip', multi_progressvar=None, localprintcommand=None ):
    ...
    hyp_ss, multi_wl = read_spectralsensitivity( spectralsensitivityfile, hyp_wl, localprintcommand=localprintcommand )
...
def resample_hyperspectral( hyp_infile, spectralsensitivitymatrix, out_pixelsize, out_multifilename=None, 
    ...
    if pixelsizereserve == None:
        ...
        pixelsizereserve = out_pixelsize
    ...
    multi_imageX = int( ( hyp_x.max()+hyp_pixelsize[0]/2 - multi_x0 ) // out_pixelsize )
    multi_imageY = int( ( multi_y0 - hyp_y.min()-hyp_pixelsize[1]/2  ) // out_pixelsize )
    ...
    for i in range( multi_imageX ):
        ...
        x = multi_x0 + i*out_pixelsize
        ii = np.where( np.logical_and( hyp_x>=x, hyp_x<x+out_pixelsize ) )[0]
    ...
    multi_mapdata[1:7] = [ str(1), str(1), str(multi_x0), str(multi_y0), str(out_pixelsize), str(out_pixelsize) ]
    ...
    for i_sl,senline in enumerate(multidata_map):
        y_i = multi_y0-i_sl*out_pixelsize
        ii = np.where( np.logical_and( hyp_y>=y_i-out_pixelsize, hyp_y<y_i ) )[0]
        i_Ystart = min(ii)
