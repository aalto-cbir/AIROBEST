"""
effective_LAI_function.py computes effective LAI for Forest Centre (Metsakeskus) stands given in geopackage format.
...
******************************************************************************************
 ...
 About this function:
    ...
    Function returns: a dictionary with standids as keys, effective LAIs as values
    ...
    Processing is done in the following steps:
       ...
       * retrieve all strata for each (tree)standid
           ...
           - sum to get effective LAI of entire stand
...
def effective_LAI(file_in, treestandid_inc):
    ...
    for species_i, leafbiomass_i, stand_i in zip( gpkg_species, gpkg_leafbiomass, standids ):   # Goes through each row of the lists in parentheses.
        ...
        if len(species_i) > 0:
            ...
            if stand_biomass > 0:
                ...
                # Sum of all stratum values represents the value of one specific treestand. Let's save the sum to the dictionary (function output)
                LAIdict[stand_i] = sum( stratum_operation )
